classdef channel_t < handle
    ...
    methods (Static = true, Access = public)
       ...
       function obj = channel_t(channel_config)
            assert(isa(channel_config, "dectnrp_channel.channel_config_t"));

            obj.channel_config = channel_config;

            if strcmp(obj.channel_config.type,'Rayleigh') || strcmp(obj.channel_config.type,'Rician')
                obj.init_Rayleigh_Rician_channel();
            end

            if obj.channel_config.sto_fractional ~= 0
                assert(obj.channel_config.spectrum_occupied <= 0.5, 'Fractional delay requires oversampling of at least 2. Set fractional delay in configuration of channel to 0.');
            end
       end
    end
    ...
    methods
        ...
        function [] = reset_random_Rayleigh_Rician(obj)
            if strcmp(obj.channel_config.type,'Rayleigh') == true || strcmp(obj.channel_config.type,'Rician') == true
        ...
        function [samples_antenna_rx] = pass_samples(obj, samples_antenna_tx, channel_time_in_seconds)
            ...
            assert(obj.channel_config.N_TX == size(samples_antenna_tx, 2));
            
            % apply amplitude
            samples_antenna_ch = obj.channel_config.amp*samples_antenna_tx;

            % apply STO, CFO and the carrier phase
            samples_antenna_ch = dectnrp_channel.sto_cfo_phase(samples_antenna_ch, ...
                                                               obj.channel_config.sto_integer, ...
                                                               obj.channel_config.sto_fractional, ...
                                                               obj.channel_config.cfo, ...
                                                               obj.channel_config.err_phase);
                    
            % over-the-air transmission
            if strcmp(obj.channel_config.type, 'AWGN')
                samples_antenna_rx = repmat(sum(samples_antenna_ch, 2), 1, obj.channel_config.N_RX);
            elseif strcmp(obj.channel_config.type, 'Rayleigh') == true || strcmp(obj.channel_config.type, 'Rician') == true
                samples_antenna_rx = obj.r_matlab_MIMO_obj(samples_antenna_ch, channel_time_in_seconds);
            else
                error('Channel neither AWGN, nor Rayleigh, nor Rician, but %s.', obj.channel_config.type);
            end
            
            % thermal noise
            for i=1:1:obj.channel_config.N_RX
                samples_antenna_rx(:,i) = awgn(samples_antenna_rx(:,i), obj.channel_config.snr_db, pow2db(1/obj.channel_config.spectrum_occupied));
            end

            if obj.channel_config.verbosity >= 1
        ...
        function [] = plot_channel_properties(obj)
            ...
            elseif strcmp(obj.channel_config.type, 'Rayleigh') == true || strcmp(obj.channel_config.type, 'Rician') == true
                % lookup PDP
                [pathDelays_beforeInterpolation, avgPathGains_beforeInterpolation] = dectnrp_channel.get_PDP_from_literature(obj.channel_config.r_type, ...
                                                                                                                             obj.channel_config.r_DS_desired);
            else
                error('Channel neither AWGN, nor Rayleigh, nor Rician, but %s.', obj.channel_config.type);
            end
    ...
    methods (Hidden = true)
        function init_Rayleigh_Rician_channel(obj)
            ...
            Ts = 1/obj.channel_config.r_samp_rate;
            ...
            if strcmp(obj.channel_config.type, 'Rayleigh') == true
                
                assert(strcmp(obj.channel_config.r_type, 'TDL-i') || ...
                       strcmp(obj.channel_config.r_type, 'TDL-ii') || ...
                       strcmp(obj.channel_config.r_type, 'TDL-iii') || ...
                       strcmp(obj.channel_config.r_type, 'InH NLOS') || ...
                       strcmp(obj.channel_config.r_type, 'UMi NLOS') || ...
                       strcmp(obj.channel_config.r_type, 'UMi OtI'));

                obj.r_matlab_MIMO_obj = comm.MIMOChannel('SampleRate', obj.channel_config.r_samp_rate, ...
                                                         'PathDelays', pathDelays, ...
                                                         'AveragePathGains', avgPathGains, ...
                                                         'NormalizePathGains', true, ...
                                                         'FadingDistribution', 'Rayleigh', ... % KFactor, DirectPathDopplerShift, DirectPathInitialPhase
                                                         'MaximumDopplerShift', obj.channel_config.r_max_doppler, ...
                                                         'DopplerSpectrum', doppler('Jakes'), ...
                                                         'SpatialCorrelationSpecification', 'Separate Tx Rx', ... %'NumTransmitAntennas', obj.channel_config.N_TX, 'NumReceiveAntennas', obj.channel_config.N_RX, ...
                                                         'TransmitCorrelationMatrix', eye(obj.channel_config.N_TX), ...
                                                         'ReceiveCorrelationMatrix', eye(obj.channel_config.N_RX), ... % SpatialCorrelationMatrix
            ...
            elseif strcmp(obj.channel_config.type, 'Rician') == true
                
                assert(strcmp(obj.channel_config.r_type, 'TDL-iv') || ...
                       strcmp(obj.channel_config.r_type, 'TDL-v') || ...
                       strcmp(obj.channel_config.r_type, 'InH LOS') || ...
                       strcmp(obj.channel_config.r_type, 'UMi LOS'));
                ...
                obj.r_matlab_MIMO_obj = comm.MIMOChannel('SampleRate', obj.channel_config.r_samp_rate, ...
                                                         ...
                                                         'KFactor', obj.channel_config.r_K, ...
                                                         'DirectPathDopplerShift',obj.channel_config.r_max_doppler*0.7, ...
