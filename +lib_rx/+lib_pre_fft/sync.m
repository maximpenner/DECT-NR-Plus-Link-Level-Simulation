function [samples_antenna_sync, sync_report] = sync(verbosity, ...
    ...
    coarse_threshold_crossing_idx = lib_rx.lib_pre_fft.detection(pre_fft_config, samples_antenna);

    %% after detection, extract the range of samples required for the upcoming steps

    A = coarse_threshold_crossing_idx + 1;
    ...
    coarse_sync_idx = lib_rx.lib_pre_fft.coarse_peak_search(verbosity, ...
                                                            pre_fft_config, ...
                                                            samples_antenna, ...
    ...
    if pre_fft_config.active_fractional == true
        ...
        [samples_antenna_stf_at_coarse_peak_cfo_fractional, ...
            fractional_cfo_report] = lib_rx.lib_pre_fft.cfo_fractional(samples_antenna_stf_at_coarse_peak, ...
                                                                       n_samples_STF_os, ...
                                                                       u);
    else
    ...
    % output container
    samples_antenna_sync = zeros(n_packet_samples, N_RX);
    ...
    if max_idx_fine_plus_packet > n_samples_antenna
        max_idx_fine = n_samples_antenna - n_packet_samples + 1;
    ...
    sync_report.fractional_cfo   = fractional_cfo_report;
