function [coarse_threshold_crossing_idx] = detection(pre_fft_config, samples_antenna)
    ...
    for i=1:1:N_RX
        ...
        for k = 1 : pre_fft_config.threshold_step : numel(metric)

            % determine the metric at this sample index
            metric(k) = lib_rx.lib_pre_fft.coarse_metric(samples_antenna_single(k : k + n_samples_STF_os - 1), M, L, pre_fft_config.minimum_power_threshold);
