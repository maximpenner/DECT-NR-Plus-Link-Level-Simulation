...
% Basically the Channel Matrix can be divied into three matrices
% V = precoding matrix
% U = receiver matrix
% S = diagonal matrix, which can be used to calculate the Rank Indicator =
% Number of Nonzero Values in S
...
% Practically the RI and the PMI are seleted according to a maximum
% calculated capacity
% E.G. https://ieeexplore.ieee.org/document/5670402

% in https://ieeexplore.ieee.org/document/6691823 and 
% https://ieeexplore.ieee.org/document/8059870
% a good calculation is shown based on LTE
% by using the channel correlation matrix which can be averaged over the 
% whole bandwidth, for this we can use the pilots (so we use the code from
% +/dectnrp_rx/channel_estimation/estimate.m)
%
% Also we need a good noise estimation, otherwise the capacity calculation
% won't work
...
%% look for the possible candidates for spatial streams (Rank Indicator) and codebook index
    ...
    if N_eff_TX < 8
        % maxmimum of transmit stream is dependant on number of receive
        % antennas
        while (N_TS_candidates(end) > N_RX)
            N_TS_candidates(end) = [];
        end
    
        % look for all poissible codebook indexes we could use
...
%% calculate the metric and pick the best candidate
        ...
        for i=1:numel(N_TS_candidates)
            ...
            % failsafe (if multiple minimums, pick the first)
